      ...
      {isLoading || isActiveOrgPending ? (
        ...
        <DropdownMenu.Root>
          <DropdownMenu.Trigger asChild>
            <button className="mt-1 flex items-center gap-1 text-sm text-muted hover:text-foreground transition-[color] outline-none">
              ...
              {orgs.map((org) => (
                ...
                >
                  <span className="w-4 shrink-0">
                    {org.id === activeOrg?.id && <CheckIcon size={14} className="text-foreground" />}
                  </span>
