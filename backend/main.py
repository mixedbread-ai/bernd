...
def save_google_tokens(fs: SemanticFS, tokens: dict):
    """Save Google OAuth tokens to mixedbread (encrypted if key configured)."""
    from datetime import datetime
    from backend.tools.crypto import encrypt_token
...
def get_google_tokens(fs: SemanticFS) -> dict | None:
    """Get stored Google OAuth tokens (decrypted if encrypted)."""
    from backend.tools.crypto import decrypt_token
